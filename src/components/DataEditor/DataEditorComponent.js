...
const DataEditorComponent = () => {
    ...
    const {
        ...
        const handleAddRow = () => {
            // Create a new blank row with default empty values
            const newRow = columns.reduce((acc, column) => {
                acc[column.accessor] = '';
                return acc;
            }, {});

            // Add the new row to the beginning of the dataset
            const updatedData = [newRow, ...tableData];

            // Update the table data
            setTableData(updatedData);

            // Reset to the first page using react-table's gotoPage method
            gotoPage(0);
        };
